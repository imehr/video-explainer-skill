...
class Orchestrator {
    ...
    memory;
    planner;
    constructor(context, config, memory) {
        this.context = context;
        this.config = config;
        this.memory = memory;
        this.planner = new planner_1.TaskPlanner(config);
    }
    /**
     ...
     */
    async executeRender(projectName, options) {
        ...
        for (const group of plan.backgroundGroups) {
            ...
            const groupResults = await Promise.all(group.tasks.map((task) => this.executeTask(task)));
     ...
     */
    async executeScript(projectName) {
     ...
     */
    async executeScenes(projectName) {
        ...
        const scenePaths = {};
        await Promise.all(aspectRatios.map(async (aspectRatio) => {
     ...
     */
    async applyFeedback(projectName, feedbackText) {
     ...
     */
    async loadApplicableSkills(stage) {
